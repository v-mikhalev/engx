...
ARA also known as application release orchestration, for exapmple, [The vRealize Code Stream utility from VMware](https://www.vmware.com/topics/glossary/content/application-release-automation.html).
...
- Конечный продукт определен и фиксирован, независимо от меняющихся потребностей рынка или технологий,
...
- Последовательное выполнение задач создает зависимости между задачами и командами, в итоге искусственно создаются проблемы занятости фиксированных комнад (простои, перегрузка).
...
Валидация (Validation) включает в себя метод динамического анализа (функциональный, нефункциональный), тестирование, выполняемое путем выполнения кода. Валидация - это процесс оценки программного обеспечения после завершения этапа разработки, чтобы определить, соответствует ли программное обеспечение ожиданиям и требованиям заказчика.
...
Одним из результатов проектирования обязательно является набор тестов, которые выполняются после кодирования:
...
- приемо-сдаточные тесты (user acceptance tests, UAT).
...
- Требования к конечному результату строго определены;
- Можно применять к крупномасштабным проектам, где объектом UAT является опытный образец;
- Главная задача определена, но детали будут уточняться в процессе.
...
Спиральная модель – это комбинация моделей Iterative и Waterfall со значительным акцентом на анализе рисков. Основная задача спиральной модели – определить правильный момент для перехода на следующий этап. Например:

- переход к следующему этапу осуществляется по плану, в заранее установленные сроки, даже если работы по предыдущему этапу еще не выполнены. План может создаваться на основе статистических данных, полученных в ходе предыдущих проектов, или из личного опыта разработчика;
...
- Большое количество промежуточных этапов требует чрезмерных (лишних) расходов.
...
- Новый продукт, который следует выпускать в несколько этапов, чтобы получить достаточно отзывов клиентов.
...
Agile methods emphasize real-time communication, preferably face-to-face, over written documents. Most agile teams include all the people necessary to finish the software. At a minimum, this includes programmers and the people who define the product such as product managers, business analysts, or actual customers. The bullpen may also include testers, interface designers, technical writers, and management .
...
Создание списка дел для одноразового проекта или переиспользование списка дела для повторяющихся проектов позволяет PM-у получить четкое представление о шагах, которые необходимо предпринять, и точных действиях, которые необходимо выполнить. Пожалуй, это самый простой и удобный способ создать схему с полным контролем над ходом проекта.
...
Каждый выполненный пункт плана в теории дает нам ощущение успеха, очередного достижения, сокращает список невыполненых работ по проекту.
...
- между контрольными точкам выполняются определенные работы, решаются определенные задачи проекта.
...
Второй уровень WBS – это контрольные точки, точки управления, которые группируют пакеты работ для измеряния статуса этих пакетов. Контрольная точка контролирует часть проекта.
...
Контрольный список (checklist), список дел (to-do list) практически тоже самое, что и WBS, выглядит чуть иначе.
